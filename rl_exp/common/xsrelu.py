...
class XSReLU_cw_std(XSReLU_cw):
    def __init__(self, std_mult=0.8, spread=0.01):
        super().__init__()
        self.spread = spread
        self.std_mult = nn.Parameter(torch.Tensor([std_mult]))
    ...
    def forward(self, input):
        ...
        r_low = F.relu(x - mean - 1 / (1 + self.spread) * self.std_mult * std)
        r_high = F.relu(x - mean - (1 + self.spread) * self.std_mult * std)
        res = r_low + (r_high - r_low) * self.std_mult

        return res.view_as(input)


class XSReLU_cw_std_2(XSReLU_cw):
    def __init__(self, std_mult=0.8, spread=0.01):
        super().__init__()
        self.spread = spread
        self.std_mult = nn.Parameter(torch.Tensor([std_mult]))
    ...
    def forward(self, input):
        ...
        r_low = F.relu(x - mean - 1 / (1 + self.spread) * self.std_mult * std)
        r_high = F.relu(x - mean - (1 + self.spread) * self.std_mult * std)
        res = r_low + (r_high - r_low) * F.sigmoid(self.std_mult)
...
class XSReLU_cw_perc_param_4(XSReLU_cw):
    def __init__(self, plogit=1.5, spread=0.01):
        super().__init__()
        self.spread = spread
        self.plogit = nn.Parameter(torch.Tensor([plogit / 5]))
