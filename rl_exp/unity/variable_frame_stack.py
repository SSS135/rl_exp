...
from rl_exp.unity.variable_env import VariableEnv
from rl_exp.unity.variable_step_result import VariableStepResult
from rl_exp.unity.variable_wrapper import VariableWrapper
...
class VariableFrameStack(VariableWrapper):
    def __init__(self, env: VariableEnv, k=4):
        super().__init__(env)
        self.k = k
        self._frames: Dict[AgentId, Deque[np.ndarray]] = defaultdict(lambda: deque([], maxlen=k))
